...
def get_ptb_format_from_diora_tree(
    parse, tokens, return_string=False, batched=False
):
    if batched:
        return [
            get_ptb_format(p, t, return_string, batched=False)
            for p, t in zip(parse, tokens)
        ]
...
def remove_punctuation_from_tree(tree, ref):
    def recursive_remove_using_mask(tr, position, mask):
        ...
        keep = []
        for x in tr:
            xsize, xkeep = recursive_remove_using_mask(
                x, position + size, mask
            )
            size += xsize
            keep.append(xkeep)
    ...
    mask = [
        x in word_tags for x in part_of_speech
    ]  # Tokens that are punctuation are given False in mask.
    new_tokens = [x for x, m in zip(tokens, mask) if m]
    assert len(new_tokens) > 0

    recursive_remove_using_mask(tree, 0, mask)
    assert len(tree.leaves()) == len(new_tokens), (
        tree.leaves(),
        new_tokens,
        tokens,
        mask,
...
def main(args):
    ...
    assert len(gold) == len(
        pred
    ), f"The gold and pred files must have same number of sentences. {len(pred)} != {len(gold)}"
    ...
    assert os.path.exists(
        evalb_exe
    ), "Did not detect evalb executable. Try `(cd EVALB && make)`."
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="File with DIORA predictions from `parse.py`.",
    )
    parser.add_argument(
        "--gold",
        type=str,
        required=True,
        help="File with ground truth parse trees in PTB format.",
    )
    parser.add_argument(
        "--out",
        type=str,
        required=True,
        help="Directory to write intermediates files for EVALB and results.",
    )
    parser.add_argument(
        "--evalb", type=str, required=True, help="Path to EVALB directory."
    )
    parser.add_argument(
        "--evalb_config",
        type=str,
        required=True,
        help="Path to EVALB configuration file.",
    )
    parser.add_argument(
        "--max_length",
        default=None,
        type=str,
        help="Max length after removing punctuation.",
    )
