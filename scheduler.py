    ...
    for i in range(n_models):
        ...
        try:
            ...
            res = pool.apply_async(
                device_wrapper, args=(i, device, func, *args), kwds=kwargs
            )
    ...
    for schedule in schedules:
        ...
        if local_option == "standard":
            ...
            for k, v in schedule["variables"].items():
                if len(v) != n_conf:
                    raise ValueError(
                        "Number of variables must be equal to number of configurations in STANDARD mode."
                    )
...
def main(args):
    ...
    args_list = [[EasyDict({"conf": n})] for n in cfgs_path]
    kwargs_list = None

    # Import execution file
    exec_path = Path(args.exec_file).stem
    exec_module = importlib.import_module(exec_path)
    sys.modules[args.function] = exec_module
    exec_func = getattr(exec_module, args.function)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        metavar="exec_file",
        type=str,
        help="A path for execution file that you want to run.",
    )
    parser.add_argument(
        "--function", "-f", default="main", help="A function name to run."
    )
