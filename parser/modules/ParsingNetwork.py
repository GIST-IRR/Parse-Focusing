...
class ParsingNetwork(nn.Module):
    ...
    def forward(self, emb, parser_state):
        ...
        emb = emb_last.permute(1, 2, 0)  # bsz, ninp, ntimestep + nlookback
    ...
    def init_hidden(self, bsz):
        ...
        self.ones = weight.new(bsz, 1).zero_() + 1
        return weight.new(self.nlookback, bsz, self.ninp).zero_(), \
               weight.new(bsz, self.nslots).zero_() + numpy.inf
    

if __name__ == "__main__":
    # emb_layer = nn.Embedding(10000, 800)

    model = ParsingNetwork(800, 1200).cuda()
    hidden = model.init_hidden(32)
    x = torch.randn(10, 32, 800).cuda()

    (memory_gate, memory_gate_next), gate, hidden = model(x, hidden)