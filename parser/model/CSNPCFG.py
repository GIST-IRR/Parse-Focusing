...
class CSNPCFG(PCFG_module):
    def __init__(self, args):
        super(CSNPCFG, self).__init__()
        self.pcfg = PCFG()
    ...
    def forward(self, input):
        ...
        root, unary, rule = self.root(), self.terms(), self.nonterms()
    ...
    def loss(self, input, partition=False, soft=False):
        ...
        nonterm_cs = self.rules['nonterm_cs'].triu(diagonal=1).abs().mean()
        term_cs = self.rules['term_cs'].triu(diagonal=1).abs().mean()
