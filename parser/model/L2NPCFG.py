...
from torch_support.metric import pairwise_cosine_similarity
...
class L2NPCFG(PCFG_module):
    def __init__(self, args):
        super(L2NPCFG, self).__init__()
        self.pcfg = PCFG()
    ...
    def forward(self, input):
        ...
        e_rule = rule.exp().unsqueeze(0)
        rule_dist = torch.cdist(e_rule, e_rule).squeeze(0)
        e_unary = unary.exp().unsqueeze(0)
        unary_dist = torch.cdist(e_unary, e_unary).squeeze(0)
    ...
    def loss(self, input, partition=False, soft=False):
        ...
        rule_dist = self.rules['nonterm_cs'].triu(diagonal=1).mean()
        unary_dist = self.rules['term_cs'].triu(diagonal=1).mean()

        return -result["partition"] \
            - self.lamb[0] * rule_dist \
            - self.lamb[1] * unary_dist
