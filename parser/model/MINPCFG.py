...
from torch_support.metric import (
    pairwise_mutual_information, n_pairwise_mutual_information
)
...
class MINPCFG(PCFG_module):
    def __init__(self, args):
        super(MINPCFG, self).__init__()
        self.pcfg = PCFG()
    ...
    def forward(self, input):
        ...
        # rule_mi = pairwise_mutual_information(rule[0], normalize=False)
        rule_mi = n_pairwise_mutual_information(rule[0], normalize=False)
        # unary_mi = pairwise_mutual_information(unary[0], normalize=True)
    ...
    def loss(self, input, partition=False, soft=False):
        ...
        rule_mi_triu = torch.triu(self.rules['rule_mi'], diagonal=1)
        # unary_mi_triu = torch.triu(self.rules['unary_mi'], diagonal=1)
        return -result["partition"] \
            + self.lamb[0] * rule_mi_triu.mean() \
            # + self.lamb[1] * unary_mi_triu.mean()
