...
class EncodingLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        term = self.term_norm(x + self.term(x)).mean(dim=1, keepdim=True)

        return term

class TermAPCFG(PCFG_module):
    def __init__(self, args):
        super(TermAPCFG, self).__init__()
        self.pcfg = PCFG()
    ...
    def forward(self, input):
        ...
        root, rule, unary = self.root(term_z), self.nonterms(term_z), self.terms(term_z)
