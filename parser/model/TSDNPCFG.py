...
from structformer.structformer import StructFormer
...
class TSDNPCFG(PCFG_module):
    def __init__(self, args):
        super(TSDNPCFG, self).__init__()
        self.pcfg = PCFG()
        ...
        # Parser options
        self.pad_token = getattr(args, "pad", 0)
        self.unk_token = getattr(args, "unk", 1)
        self.mask_token = getattr(args, "mask", 2)

        # Parser
        # with open("structformer/model/model_v2.pt", "rb") as f:
        #     self.parser, _, _, _ = torch.load(f)

        self.parser = StructFormer(512, 8, self.V)

        self.parser_criterion = nn.CrossEntropyLoss(ignore_index=self.pad_token)
    ...
    def mask_data(self, data):
        ...
        mask = mask_bernoulli.sample(data.shape).to(data.device).bool()
        mask = mask.squeeze(-1)
        mask = mask * (data != self.pad_token) * (data != self.unk_token)
        targets = data.masked_fill(~mask, self.pad_token)
        data = data.masked_fill(mask, self.mask_token)
        return data, targets
    ...
    def loss(self, input, partition=False, soft=False):
        ...
        # Mask input sequence
        x, targets = self.mask_data(words)
        pos = torch.arange(seq_len, device=words.device)[None, :]

        # Sentence pre-parser
        output, p_dict = self.parser(x, pos)
        ...
        parser_loss = self.parser_criterion(output, targets.reshape(-1))

        return -result["partition"] + parser_loss
