...
class Inside(torch.autograd.Function):
    ...
    def forward(ctx, noninherent_word, noninherent_symbol, inherent, root, head, span_indicator, arc_indicator, lens):
        ...
        for w in range(2, N):
            ...
            diagonal_copy_with_headword(
                s_normalizer, headed_normalizer[..., None], w)
...
class BLPCFG(PCFG_base):
    ...
    def _inside_one(
        ...
        for w in range(2, N):
            ...
            width_idx = root.new_ones(batch*n).bool()
            ts = [
                s[0].item() + i*n
            ...
            width_idx[ts] = False
            width_idx = width_idx.reshape(batch, n, 1, 1, 1)
            closed_idx = width_idx.reshape(batch, n, 1, 1)

            # Equation (8).  Term D1-1 \times Term D1-2
            # (b, n, w, H, r)
            left = stripe_with_headword(s_inherent, n, w - 1, (0, 1))
            right = stripe_with_headword(s_inherent, n, w - 1, (1, w), 0)
            left = left.masked_fill(width_idx, -1e9)
            right = right.masked_fill(width_idx, -1e9)
            # (b, n, w, r)
            closed_left = stripe(s_noninherent[..., LEFT], n, w - 1, (0, 1))
            closed_right = stripe(s_noninherent[..., RIGHT], n, w - 1, (1, w), 0)
            closed_left = closed_left.masked_fill(closed_idx, -1e9)
            closed_right = closed_right.masked_fill(closed_idx, -1e9)
            # (b, n, H, A)
            headed = merge(
                left.clone(), right.clone(),
                closed_left.clone(), closed_right.clone(),
                stripe_grammar_rules(head, n, w)
            )
            headed = headed + span_indicator[:, torch.arange(n), torch.arange(n) + w, None, None]
            diagonal_copy_with_headword(s, headed, w)
    ...
    def loss(self, rules, lens, **kwargs):
        if 'tree' in kwargs:
            return self._inside_one(rules, lens, decoding=False, **kwargs)
        else:
    ...
    def _mbr_arc(self, rules, lens):
        ...
        with torch.enable_grad():
            ...
            s_noninherent_right[:, torch.arange(H), torch.arange(H) + 1, :,  :] = noninherent[:, :, t_slice, :, RIGHT].logsumexp(-2).unsqueeze(-2) + arc_indicator[:, torch.arange(H), :].unsqueeze(-1)
