...
def checkpoint(func):
    def wrapper(*args, **kwargs):
        return ckp(func, *args, **kwargs, use_reentrant=True)
...
def stripe_with_headword(x, n, w, offset=(0, 0), dim=1):
    ...
    stride[2] = (1 if dim == 1 else seq_len) * numel
    return x.as_strided(
        size=(x.shape[0], n, w, w + 1, *list(x.shape[4:])),
        stride=stride,
        storage_offset=(offset[0] * seq_len + offset[1]) * numel,
    )
...
def stripe_grammar_rules(x, n, w, offset=0):
    ...
    return x.as_strided(
        size=(x.shape[0], n, w, *x.shape[2:]),
        stride=new_stride,
        storage_offset=offset * stride[1],
    )
...
def diagonal_copy_depth_old(x, y, w, d):
    ...
    if len(x.shape) > 3:
        ...
        x.as_strided(
            size=(x.shape[0], seq_len - w, *list(x.shape[3:-1]), d),
            stride=new_stride,
...
def diagonal_copy_depth(x, y, w, d):
    ...
    if len(x.shape) > 3:
        new_stride.extend(stride[3:])
        x.as_strided(
            size=(x.shape[0], seq_len - w, *list(x.shape[3:-1]), d_size),
            stride=new_stride,
...
def diagonal_depth(x, w, d):
    ...
    if len(x.shape) > 3:
        new_stride.extend(stride[3:])
        return x.as_strided(
            size=(x.shape[0], seq_len - w, d_size),
            stride=new_stride,
            storage_offset=(w * stride[2] + min_d * stride[-1]),
        )
    else:
...
def diagonal_copy_with_headword(x, y, w):
    ...
    if len(x.shape) > 4:
        ...
        x.as_strided(
            size=(x.shape[0], seq_len - w, w, *list(x.shape[4:])),
            stride=new_stride,
            storage_offset=w * stride[2],
        ).copy_(y)
    else:
        new_stride.append(stride[3])
        x.as_strided(
            size=(x.shape[0], seq_len - w, w, *list(x.shape[3:])),
            stride=new_stride,
            storage_offset=w * stride[2],
        ).copy_(y)
...
def stripe_headed_left(x, n, w, nt, t):
    ...
    new_stride.extend(stride[3:])
    return x.as_strided(
        size=(x.shape[0], n, w, x.shape[3], nt, t),
        stride=new_stride,
...
def stripe_headed_right(x, n, w, nt, t):
    ...
    new_stride.extend(stride[3:])
    return x.as_strided(
        size=(x.shape[0], n, w, x.shape[3], t, nt),
        stride=new_stride,
...
def stripe_need_dad(x, n, w, start, end, headstart):
    ...
    new_stride.extend(stride[4:])
    return x.as_strided(
        size=(x.shape[0], n, w, *list(x.shape[4:])),
        stride=new_stride,
